...
//#region src/lib/server/ai.js
var summarizeBrand = (data) => {
	const settings = Object.fromEntries((data.app_settings || []).map((s) => [s.key, s.value]));
	const products = (data.products || []).filter((p) => p.is_active).map((p) => `- ${p.name}: Rp${Number(p.price).toLocaleString("id-ID")}/${p.unit}${p.description ? ` (${p.description})` : ""}`).join("\n");
	const promos = (data.promotions || []).filter((p) => p.is_active).map((p) => {
...
var aiSystemPrompt = (brandDoc) => `[INSTRUKSI UTAMA - JANGAN DIUBAH]
...
${brandDoc}
[/DOKUMEN]`;
async function callAI(userMessage, brandDoc) {
	const s = await getSettings();
	const AI_BASE_URL = s.ai_base_url;
	const AI_API_KEY = s.ai_api_key;
	const AI_MODEL = s.ai_model;
	if (!AI_BASE_URL || !AI_API_KEY || !AI_MODEL) return "Maaf, layanan AI sedang tidak tersedia.";
	try {
		return (await (await fetch(`${AI_BASE_URL}/chat/completions`, {
			method: "POST",
			headers: {
			...
			body: JSON.stringify({
				...
				messages: [{
					role: "system",
					content: aiSystemPrompt(brandDoc)
				}, {
					role: "user",
					content: userMessage
				}],
		...
		})).json())?.choices?.[0]?.message?.content || "Maaf, tidak ada jawaban dari AI.";
	} catch (e) {
...
async function getBrandData() {
	...
	return {
		app_settings: appSettings.rows.map((r) => ({
			key: r.setting_key,
			value: r.setting_value
		})),
		faqs: faqs.rows.map((r) => ({
			id: r.faq_id,
		...
		promotions: promotions.rows.map((r) => ({
			name: r.promo_name,
			type: r.promo_type,
			value: r.promo_value,
			code: r.promo_code,
			is_active: r.promo_is_active === 1
		}))
	};
}
async function sendTelegram(chatId, message) {
	const TELEGRAM_BOT_TOKEN = (await getSettings()).telegram_bot_token;
...
function estimatePickup(settings, userLoc) {
	const store = {
	...
	return {
		...
		freeRadiusKm: radius,
		billableKm: billable,
		fee: Math.round(billable * perKm)
	};
}
//#endregion
export { summarizeBrand as a, sendTelegram as i, estimatePickup as n, getBrandData as r, callAI as t };
