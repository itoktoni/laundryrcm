...
//#region src/routes/api/finance/export/+server.js
async function GET({ url }) {
	...
	return generateCsv(transactions.rows, {
		income,
		expense,
		profit,
		unpaid: totalUnpaid
...
function generateCsv(rows, summary) {
	...
	data.push({
		...
		Jumlah: summary.profit,
		Keterangan: ""
	});
	data.push({
		Tanggal: "",
		Tipe: "",
		Kategori: "PIUTANG (BELUM DIBAYAR)",
		Jumlah: summary.unpaid,
		Keterangan: ""
	});
	return csvResponse("laporan-keuangan.csv", toCsv(columns, data));
}
//#endregion
export { GET };
