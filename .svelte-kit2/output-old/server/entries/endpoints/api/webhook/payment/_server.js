...
//#region src/routes/api/webhook/payment/+server.js
var logDir = join(process.cwd(), "logs");
var logFile = join(logDir, "webhook.log");
var dirReady = false;
async function log(...args) {
	const line = `[webhook/payment] ${(/* @__PURE__ */ new Date()).toISOString()} ${args.map((a) => typeof a === "object" ? JSON.stringify(a) : a).join(" ")}\n`;
...
async function POST({ request }) {
	try {
		...
		await log("received request", {
			debug,
			method: request.method,
			contentType: request.headers.get("content-type")
		});
		const headers = {};
		for (const [key, value] of request.headers) if (key.startsWith("x-")) headers[key] = value;
		await log("x-headers", headers);
		const authHeader = request.headers.get("authorization") || "";
		let authUser = "";
		let authPass = "";
		let authMethod = "none";
		if (authHeader.startsWith("Basic ")) try {
		...
		if (!authorized && authMethod === "basic" && expectedUser && expectedPass) authorized = authUser === expectedUser && authPass === expectedPass;
		if (!authorized && authMethod === "bearer" && expectedPass) authorized = authPass === expectedPass;
		if (!authorized && authMethod === "x-api-key" && expectedPass) authorized = authPass === expectedPass;
		if (!authorized) {
		...
		await log("payload", {
			...
			extractedAmount,
			notifText,
			unique_code: body.unique_code ?? body.uniqueCode ?? (body.source ? String(body.source).replace(/\s+/g, "_").toLowerCase() : null)
		});
		...
		/** @type {any} */
		let result = { rows: [] };
		if (reference) result = await db.execute({
			sql: `SELECT * FROM orders
					WHERE order_id = ?
					LIMIT 1`,
			args: [reference]
		});
		if (result.rows.length === 0) result = await db.execute({
			sql: `SELECT * FROM orders
		...
		if (result.rows.length === 0) {
			await log("no matching order at all", {
				amount: amountNum,
				reference
			});
		...
		await log("marking paid", {
			order_id,
			payment_code: paymentCode,
			amount: orderPaidAmount,
			psp_amount: amountNum
		});
		await db.execute({
