...
//#region src/routes/inventory/[id]/+page.server.js
async function load({ params }) {
	...
	if (inventoryResult.rows.length === 0) throw error(404, "Item tidak ditemukan");
	...
	const formattedMovements = movements.map((m, i) => {
		...
		if (type === "out" && movementAvgCost > 0) currentAvg = movementAvgCost;
		return {
			...m,
			sign: type === "in" ? "+" : "-",
			nominal: cost,
