...
//#region src/routes/inventory/+page.server.js
async function load() {
...
var actions = {
	add: async ({ request }) => {
		...
		if (!name || isNaN(quantity) || !unit || isNaN(minStock) || isNaN(price) || price < 0) return { error: "Semua field wajib diisi" };
	...
	stockOut: async ({ request }) => {
		...
		const currentQty = current.rows[0].inventory_quantity || 0;
		const currentAvgCost = current.rows[0].inventory_avg_cost || 0;
		if (qty > currentQty) return { error: "Stok tidak mencukupi" };
		const newQty = currentQty - qty;
	...
	updateStock: async ({ request }) => {
		...
		if (!id || isNaN(quantity)) return { error: "Data tidak valid" };
		if (minStock !== null && !isNaN(minStock)) await db.execute({
	...
	editPrice: async ({ request }) => {
		...
		if (!id || isNaN(price) || price < 0) return { error: "Harga tidak valid" };
		await db.execute({
	...
	edit: async ({ request }) => {
		...
		const unit = formData.get("unit")?.toString();
		const minStock = parseFloat(formData.get("min_stock"));
		if (!id || !name || !unit || isNaN(minStock)) return { error: "Data tidak valid" };
		await db.execute({
	...
	delete: async ({ request }) => {
		...
		await db.execute({
			sql: "DELETE FROM stock_movements WHERE inventory_id = ?",
			args: [id]
		});
		await db.execute({
