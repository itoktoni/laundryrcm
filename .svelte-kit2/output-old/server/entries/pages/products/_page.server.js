...
//#region src/routes/products/+page.server.js
...
var actions = {
	add: async ({ request }) => {
		...
		if (!name || isNaN(price) || !unit) return fail(400, { error: "Nama, harga, dan satuan wajib diisi" });
		await db.execute({
	...
	edit: async ({ request }) => {
		...
		const categoryId = formData.get("category_id")?.toString() || null;
		const isActive = formData.get("is_active") === "1" ? 1 : 0;
		if (!id || !name || isNaN(price) || !unit) return fail(400, { error: "Data tidak valid" });
		await db.execute({
	...
	delete: async ({ request }) => {
		...
		if ((await db.execute({
			...
			args: [id]
		})).rows[0]?.count > 0) return fail(400, { error: "Produk tidak bisa dihapus karena masih ada di order" });
		await db.execute({
