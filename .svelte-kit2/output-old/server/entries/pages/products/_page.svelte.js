		...
		if (filtered().length === 0) {
			...
			for (let $$index_3 = 0, $$length = each_array_2.length; $$index_3 < $$length; $$index_3++) {
				...
				if (editId === product.product_id) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<form method="POST" action="?/edit" class="mt-3 space-y-2 border-t border-outline-variant pt-3"><input type="hidden" name="id"${attr("value", product.product_id)}/> <input type="text" name="name"${attr("value", product.product_name)} required="" placeholder="Nama Produk" class="w-full h-11 px-4 bg-surface-container-lowest border border-outline-variant rounded-lg text-body-sm"/> <div class="grid grid-cols-2 gap-2"><input type="number" name="price" step="0.01"${attr("value", product.product_price)} required="" placeholder="Harga" class="h-11 px-4 bg-surface-container-lowest border border-outline-variant rounded-lg text-body-sm"/> `);
					$$renderer.select({
						name: "unit",
