...
//#region src/routes/promo/+page.server.js
async function load() {
	return { promotions: (await db.execute("SELECT * FROM promotions ORDER BY promo_created_at DESC")).rows };
}
var actions = {
	add: async ({ request }) => {
		...
		if (!name || !type || isNaN(value) || !startDate || !endDate) return { error: "Semua field wajib diisi" };
		await db.execute({
	...
	edit: async ({ request }) => {
		...
		const startDate = formData.get("start_date")?.toString();
		const endDate = formData.get("end_date")?.toString();
		if (!id || !name || !type || isNaN(value) || !startDate || !endDate) return { error: "Semua field wajib diisi" };
		await db.execute({
