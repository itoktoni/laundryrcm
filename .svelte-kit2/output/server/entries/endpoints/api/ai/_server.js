...
//#region src/routes/api/ai/+server.js
async function GET({ request }) {
	const { getSettings } = await import("../../../../chunks/settings.js");
	const expectedKey = (await getSettings()).ai_api_key || "";
	const authHeader = request.headers.get("authorization") || "";
	if (!expectedKey || !authHeader.startsWith("Bearer ") || authHeader.slice(7) !== expectedKey) return json({ error: "Unauthorized" }, { status: 401 });
	...
	return json({
		app_settings: appSettings.rows.filter((row) => row.status !== "private").map((row) => ({
			key: row.setting_key,
