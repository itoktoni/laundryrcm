...
async function checkOverduePickup() {
    ...
    `, [pendingDays]);
		for (const o of orders) {
			...
			const template = crmSettings.crm_reminder_template_pending || TEMPLATES.overdue_pickup({
...
async function checkInactiveCustomer() {
    ...
    `, [inactiveDays]);
		for (const c of customers) {
			...
			const days = Math.floor(diffMs / 864e5);
			const template = crmSettings.crm_reminder_template_inactive || TEMPLATES.inactive_customer({
...
async function checkLowStock() {
    ...
    `);
		for (const i of items) results.push({
			phone: null,
...
async function checkMachineService() {
    ...
    `);
		for (const m of machines) results.push({
			phone: null,
...
async function checkPendingPickup() {
    ...
    `);
		for (const o of orders) {
			(o.order_notes || "").toLowerCase().includes("kirim");
			const diffMs = Date.now() - new Date(o.order_created_at).getTime();
			const days = Math.floor(diffMs / 864e5);
			if (days >= pendingDays) {
...
async function sendWhatsApp(phone, message) {
	const { WACLI_PATH, WA_SESSION_NAME } = await import("../../../../../chunks/private.js").then((m) => m.env).catch(() => ({}));
	...
	try {
		...
		if (!(await import("fs")).default.existsSync(wacliPath)) {
			console.log("[schedule] wacli not found at", wacliPath, "- skipping send to", phone);
			return true;
		}
		execSync(`"${wacliPath}" --session "${sessionName}" send text --phone "${phone}" --file "${tempFile}"`, {
			timeout: 3e4,
			stdio: "pipe"
		});
	...
	try {
		...
		if (adminPhone) {
			...
			for (const scheduleType of types) {
				...
				if (!checker) continue;
				const messages = await checker();
				for (const msg of messages) if (!msg.phone) alerts.push(`⚠️ *[${scheduleType}]*\n${msg.message}`);
			}
			if (alerts.length > 0) {
		...
		let totalQueued = 0;
		for (const msg of pendingInserts) try {
			await db.execute({
      ...
      LIMIT 10
    `);
		for (const qm of queuedMessages) try {
