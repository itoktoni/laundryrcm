...
//#region src/routes/customers/[id]/+page.server.js
...
async function load({ params }) {
	...
	if (customer.rows.length === 0) throw redirect(302, "/customers");
	const lastOrderDate = lastOrder.rows[0]?.last_order;
	const daysSinceLastOrder = lastOrderDate ? Math.round((Date.now() - new Date(lastOrderDate).getTime()) / (1e3 * 60 * 60 * 24)) : null;
	const ws = weightStats.rows[0] || {
		total_kg: 0,
		total_pcs: 0,
		total_orders: 0
	};
...
var actions = {
	...
	updateCustomer: async ({ params, request }) => {
		...
		await ensureColumns();
		try {
			await db.execute(`ALTER TABLE customers ADD COLUMN customer_est_freq_days INTEGER`);
		} catch {}
		try {
			await db.execute(`ALTER TABLE customers ADD COLUMN customer_est_weight REAL`);
		} catch {}
		await db.execute({
