...
//#region src/routes/machines/+page.server.js
async function load() {
...
var actions = {
	...
	recordService: async ({ request }) => {
		...
		const nextService = formData.get("next_service")?.toString();
		console.log("recordService action called with:", {
			id,
			description,
			cost,
			notes,
		...
		try {
			...
			return { error: "Gagal menyimpan: " + (err instanceof Error ? err.message : "Unknown error") };
		}
	},
	edit: async ({ request }) => {
		const formData = await request.formData();
		...
		const type = formData.get("type")?.toString();
		const nextService = formData.get("next_service")?.toString();
		if (!id || !name || !type) return { error: "Nama dan tipe wajib diisi" };
		await db.execute({
