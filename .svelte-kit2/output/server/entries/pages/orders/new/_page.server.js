...
import { t as recalculateCustomerProfile } from "../../../../chunks/customer-profile.js";
import { fail, redirect } from "@sveltejs/kit";
//#region src/routes/orders/new/+page.server.js
async function load() {
...
async function createOrderCore({ request, locals, forceUnpaid = false, withUniqueCode = false }) {
	...
	if (!customerId) return { error: fail(400, { error: "Pilih pelanggan" }) };
	let items;
	try {
	...
	if (!items || items.length === 0) return { error: fail(400, { error: "Tambahkan minimal 1 item" }) };
	let subtotal = 0;
	...
	if (promoCode) {
		...
		if (promo.rows.length > 0) {
			...
			if (!p.promo_min_order || subtotal >= p.promo_min_order) {
				if (p.promo_type === "percent") discount = Math.round(subtotal * (p.promo_value / 100));
				else if (p.promo_type === "nominal") discount = p.promo_value;
	...
	if (!appliedPromoId) {
		...
		if (customer.rows.length > 0 && customer.rows[0].customer_vip) discount = Math.round(subtotal * .05);
	}
	...
	if (paymentStatus === "paid") await db.execute({
	...
	for (const item of orderItems) await db.execute({
