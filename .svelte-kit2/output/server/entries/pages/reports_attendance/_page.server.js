...
//#region src/routes/reports_attendance/+page.server.js
async function load({ url }) {
	...
	if (period === "custom" && start && end) {
		...
		args = [start, end];
	} else if (period === "today") clause = "AND DATE(a.created_at) = DATE('now')";
	else if (period === "week") clause = "AND DATE(a.created_at) >= DATE('now', 'weekday 0', '-7 days')";
	else if (period === "month") clause = "AND strftime('%Y-%m', a.created_at) = strftime('%Y-%m', 'now')";
	if (userId) {
