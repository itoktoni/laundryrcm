import { a as ensure_array_like, o as head, w as escape_html } from "../../../chunks/server.js";
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="space-y-stack-lg"><section class="relative overflow-hidden rounded-3xl bg-brand-gradient p-5 text-white shadow-fab animate-fade-slide-up"><div class="absolute -right-8 -top-10 w-32 h-32 rounded-full bg-white/10"></div> <div class="absolute right-10 -bottom-8 w-20 h-20 rounded-full bg-white/10"></div> <div class="relative flex items-center gap-4"><div class="w-16 h-16 rounded-2xl bg-white/20 backdrop-blur ring-2 ring-white/30 flex items-center justify-center text-[26px] font-extrabold">${escape_html(data.user?.name?.charAt(0)?.toUpperCase() || "U")}</div> <div class="min-w-0"><h1 class="text-[20px] font-extrabold tracking-tight truncate">${escape_html(data.user?.name)}</h1> <p class="text-[12px] text-white/80 truncate">${escape_html(data.user?.email)}</p> <span class="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full bg-white/20 text-[10px] font-bold uppercase tracking-wider capitalize"><span class="material-symbols-outlined text-[12px]">badge</span> ${escape_html(data.user?.role)}</span></div></div></section> <section class="app-card p-stack-md animate-fade-slide-up"><h2 class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider mb-4">Informasi Akun</h2> <div class="space-y-3"><div class="flex items-center gap-3 p-3 bg-surface rounded-lg border border-outline-variant"><span class="material-symbols-outlined text-on-surface-variant">person</span> <div><p class="font-body-md text-on-surface">Nama</p> <p class="text-label-md text-on-surface-variant">${escape_html(data.user?.name)}</p></div></div> <div class="flex items-center gap-3 p-3 bg-surface rounded-lg border border-outline-variant"><span class="material-symbols-outlined text-on-surface-variant">email</span> <div><p class="font-body-md text-on-surface">Email</p> <p class="text-label-md text-on-surface-variant">${escape_html(data.user?.email)}</p></div></div> <div class="flex items-center gap-3 p-3 bg-surface rounded-lg border border-outline-variant"><span class="material-symbols-outlined text-on-surface-variant">shield</span> <div><p class="font-body-md text-on-surface">Role</p> <p class="text-label-md text-on-surface-variant capitalize">${escape_html(data.user?.role)}</p></div></div></div> `);
		...
		$$renderer.push(`<!--]--></section> <section class="app-card p-stack-md animate-fade-slide-up"><div class="flex items-center justify-between mb-4"><h2 class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Keamanan</h2> <button type="button" class="text-label-md text-primary font-bold">${escape_html("Ubah Password")}</button></div> `);
		$$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--></section> <section class="app-card p-stack-md animate-fade-slide-up"><h2 class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider mb-4">Tentang</h2> <div class="space-y-2 text-body-sm text-on-surface-variant"><p>LaundryKu v2.0</p> <p>Sistem Manajemen Laundry berbasis AI</p> <p>Gratis untuk semua pemilik laundry</p></div></section> <form method="POST" action="/logout"><button type="submit" class="pressable w-full h-13 flex items-center justify-center gap-2 bg-error-container text-error rounded-2xl font-bold text-[14px]"><span class="material-symbols-outlined">logout</span> Keluar dari Akun</button></form></div>`);
	});
}
//#endregion
