...
export async function GET({ url }) {
	...
	try {
		const [inactiveCustomers, pendingPickupOrders, customerStats, topCustomers] = await Promise.all([
			db.execute(`
		...
		return json({
			...
			stats: customerStats.rows[0] || { total_customers: 0, vip_count: 0, new_customers: 0, vip_percentage: 0 },
